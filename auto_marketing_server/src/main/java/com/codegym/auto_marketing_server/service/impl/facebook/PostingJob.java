...
public class PostingJob {
    ...
    @Transactional
    public void run() {
        List<ScheduledPost> scheduledPostList = scheduledPostRepository.findByStatusAndScheduledTimeLessThanEqual(
        ...
        for (ScheduledPost scheduledPost: scheduledPostList) {
            ...
            try {
                ...
                for (PostTarget postTarget: postTargetList){
                    ...
                    boolean ok = facebookClient.publishText(pageId,pageToken,message);
                    allOk = allOk&&ok;
                ...
                logger.error("Schedule {} failed: {}", scheduledPost.getId(),e.getMessage());
