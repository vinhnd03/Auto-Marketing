...
import { fetchPlans } from './../../service/pricingService';
...
export default function TransactionSuccess() {
  ...
  useEffect(() => {
    const loadPlans = async () => {
      try {
        const data = await fetchPlans(); // gọi API từ service
        setPlanList(data);
      } catch (error) {
  ...
  return (
    <div className="bg-gray-50 py-4 sm:py-8">
      <div className="max-w-full sm:max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-4 sm:mb-6 px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {planList
                .filter((plan) => plan.price > 0) // lọc gói có giá > 0
                .map((plan) => (
        ...
        <div className="hidden sm:block overflow-x-auto bg-white shadow-lg rounded-lg border border-gray-200">
          <table className="min-w-full text-sm sm:text-base border-collapse border-separate border-spacing-y-2">
                ...
                transactions.map((tx) => (
                  <tr
                    ...
                    title={`Mã GD: ${tx.transactionCode}\nGói: ${
                      tx.plan.name
                    }\nNgày: ${new Date(tx.createdAt).toLocaleDateString(
                      "vi-VN"
        ...
        <div className="sm:hidden">
          {transactions?.length > 0 ? (
            transactions.map((tx) => (
              ...
              >
                <p>
                  <strong>💳 Mã Thanh Toán:</strong> {tx.transactionCode}
                </p>
                <p>
                  <strong>📦 Gói Thanh Toán:</strong> {tx.plan.name}
                </p>
                <p>
                ...
                <p>
                  <strong>💰 Số tiền:</strong> {tx.amount.toLocaleString()} VND
                </p>
              </div>
            ))
          ) : (
