...
import { getSocialAccountByUserId } from "../../service/publish/socialAccountService";
...
const CreateWorkspaceModal = ({ isOpen, onClose, onAdd, workspaces }) => {
  ...
  useEffect(() => {
    const fetchSocialAccount = async () => {
      if (!user?.id) return;
      try {
        const account = await getSocialAccountByUserId(user.id);
        if (!account) {
          toast.error("Chưa có social account, vui lòng thêm trước");
          return;
        }
    ...
    if (isOpen && user?.id) fetchSocialAccount();
  }, [isOpen, user]);

  // Yup schema với kiểm tra trùng tên + dung lượng file
  const WorkspaceSchema = Yup.object().shape({
    name: Yup.string()
      ...
      .test("unique-name", "Tên workspace đã tồn tại", function (value) {
        ...
        return !workspaces.some(ws => ws.name.trim().toLowerCase() === lower);
      }),
        ...
        >
          {({ setFieldValue, isSubmitting, errors, touched }) => (
            <Form className="space-y-4">
              ...
              {/* Tên Workspace */}
              <div>
                <label htmlFor="workspace-name" className="block text-gray-700 font-medium text-sm">
              ...
              {/* Mô tả */}
              <div>
                <label htmlFor="workspace-description" className="block text-gray-700 font-medium text-sm">
