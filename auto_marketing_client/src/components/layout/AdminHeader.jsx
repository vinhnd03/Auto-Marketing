...
//l
        ...
        async function fetchData() {
            try {
                const [notificationsData, count] = await Promise.all([
                    getNotifications(user.id), // truyền userId
                    getUserCount(),
                ]);
    ...
    const handleMarkAllAsRead = async () => {
        try {
            await markAllNotificationsAsRead(user.id); // gọi API
            setNotifications((prev) =>
            ...
            >
                <div className="flex items-center justify-between h-full px-4 md:px-6 text-white">
                    ...
                    <div className="flex items-center space-x-4">
                        <div
                            className="hidden sm:flex items-center space-x-2 bg-white/20 text-white px-3 py-1 rounded-full">
                            <Shield size={14}/>
                                            ...
                                            {notifications.length > 0 ? (
                                                notifications.slice(0, 7).map((notification, index) => (
                                                    ...
                                                    >
                                                       ...
                                                       Ngày :   {new Date(notification.createdAt).toLocaleDateString("vi-VN")}
